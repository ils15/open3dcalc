...
**Status:** em revisão. A Themis **bloqueou** o commit `def8080` com dois achados HIGH e um LOW; as correções estão no branch `fix/toast-contrast-and-guard-floor`, em um commit de código próprio, que roda os seis gates de código do zero. Os gates anteriormente citados para `def8080` são os resultados da **versão bloqueada** e não descrevem a árvore atual. O branch foi criado a partir de `main` em `e682f09`. **Revisão e merge pendentes** — não mesclado em `main` e não liberado. Onda independente: não substitui, não reordena nem absorve nenhuma fatia da Phase 7o, da Phase 7n ou do port visual da Beta 5, e não fecha o gate transversal de compatibilidade v2.0.
...
**2. Piso da população adiada — de arquivos para sítios.** O piso prendia **contagem de arquivos**, que não enxerga um sítio mudando de _forma_: migrar um pareamento quebrado para outro pareamento quebrado diferente deixa todas as contagens de arquivos intactas, e o piso reportava verde sobre uma população igualmente quebrada. O piso passa a ser contado em **sítios**, e o pin é **por arquivo**, não global.

A primeira versão deste stage ainda usava um conjunto **global** de formas permitidas, e a Themis apontou — com razão — que ele era mais fraco do que parecia: um sítio podia migrar de uma forma já permitida para **outra** forma já permitida, sem mudar a contagem nem o conjunto, e passar em silêncio. O allowlist dizia "esta forma está quebrada" quando o que precisava dizer era "esta forma está quebrada **aqui**".

O pin agora é `arquivo -> multiconjunto de formas`, e um sítio só é coberto se **o próprio arquivo dele** ainda tem aquela forma disponível. Multiconjunto, não conjunto: o `CatalogTab.tsx` tem três sítios idênticos e isso são três informações, não uma.

**Como a identidade do sítio se mantém estável.** A identidade é o **par (arquivo, forma)**, e a estabilidade é estrutural, não mantida à mão: um sítio é _definido_ por onde está e por que forma tem, então alterar qualquer um dos dois componentes necessariamente produz uma chave diferente, e a verificação é feita sobre chaves. Não existe identificador separado para desatualizar — nem número de linha, que qualquer edição acima deslocaria, nem contador, que qualquer inserção acima renumeraria. E a codificação não pode derivar em silêncio: um sítio que muda de forma vira chave nova e é excedente; um sítio novo é chave nova; um sítio resolvido apenas deixa de ser descoberto, que é o único caso aceito.

**Prova por mutação, com o número medido.** O `SpoolThumb.tsx` foi migrado de `bg-[var(--color-accent)]/20 + text-[var(--color-accent)]` para `bg-[var(--accent)]/20 + text-[var(--accent)]` — a string **exatamente** que o `Select.tsx` já tem pinada. O veredito da lógica antiga, **medido** nos dois estados: 15 sítios, 8 formas distintas, 0 não-contabilizadas, **PASS** — antes e depois da migração, ou seja, a troca é completamente invisível para ela. O pin por arquivo reprova, nomeando o `SpoolThumb.tsx`.
