...
Estas sete estão **commitadas** e dirigem o scanner de produção; não há teste de mutação commitado neste repositório, e a garantia é de que elas **reprovam** quando a posse volta a ser último-escritor-vence, quando o cheque de órfão some, ou quando a detecção de duplicado some — verificado por mutação manual no commit que as introduziu, e reconferível a qualquer momento reaplicando as três alterações.
