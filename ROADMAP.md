...
**Status:** em revisão, com duas rodadas de bloqueio. A Themis bloqueou `def8080` (2 achados HIGH + 1 LOW) e depois `53c1d00` (3 MEDIUM + 2 LOW); todos foram corrigidos. **Commit de código cujos gates valem: `1c7b6257dcd8df8b16ce3b4b2a80d11472a23cbc`** — é a árvore em que os seis gates de código rodaram: `test:run` **0** (236 arquivos / **3274** testes), `test:run --coverage` **0** (82,97% de statements, 84,04% de linhas), `typecheck` **0**, `typecheck:electron` **0**, `lint` **0**, `build:all` **0**. Este commit de documentação segue esse commit de código e **não** é ele próprio gateado; nenhum resultado acima é atribuído a ele. O branch foi criado a partir de `main` em `e682f09`. **Revisão e merge pendentes** — não mesclado em `main` e não liberado. Onda independente: não substitui, não reordena nem absorve nenhuma fatia da Phase 7o, da Phase 7n ou do port visual da Beta 5, e não fecha o gate transversal de compatibilidade v2.0.
...
**2. Pilo da população adiada — de arquivos para ocorrências.** O piso prendia **contagem de arquivos**, que não enxerga um sítio mudando de _forma_: migrar um pareamento quebrado para outro pareamento quebrado diferente deixa todas as contagens intactas, e o piso reportava verde sobre uma população igualmente quebrada.
...
O pin passou a ser `identidade -> forma`: a chave é `arquivo#declaração#ordinal` — _qual_ sítio — e o valor é a forma que ele tem hoje — _o que_ ele é. Identidade e forma são separadas de propósito: a identidade sobrevive a reformatação, a forma é justamente o que pode mudar.

**Prova por mutação, com o número medido.** A ocorrência `CatalogTab.tsx#PrinterManager#1` foi resolvida e a **mesma** forma reintroduzida dentro de `MarketplaceManager`. No arquivo: contagem **3 → 3** e multiconjunto **`{accent/20 x3}` → `{accent/20 x3}`**, byte a byte idêntico — invisível para tudo que o pin anterior conseguia ver. O pin por ocorrência reprova e nomeia `CatalogTab.tsx#MarketplaceManager#2`.

**Estabilidade da identidade, dita sem exagero.** A chave é a declaração de coluna zero mais um ordinal base 1 dentro dela. **Sobrevive** a deslocamento de linha, edições acima do sítio, reformatação, e literais que não são pareamentos de wash. **Muda** quando um pareamento é adicionado, removido, reordenado dentro da sua declaração, ou passa a outra forma. Número de linha foi rejeitado por instável; o texto do literal foi rejeitado porque duas ocorrências de uma mesma forma ficam indiscerníveis — que é exatamente a lacuna que se está fechando; uma anotação estável na fonte foi rejeitada por invasiva, porque significaria editar treze componentes para carregar um registro de que não precisam, por uma garantia pouco mais forte que esta. **O custo, real:** inserir um pareamento no topo de uma declaração renumera os seguintes e reporta todos eles — deliberado, para que reordenar os sites quebrados conhecidos de um arquivo seja uma falha alta e mecânica de consertar, e não um rebaselineamento silencioso.

O pin continua **allowlist de uma direção só**: uma ocorrência resolvida deixa entrada obsoleta e não gera falha, porque corrigir um sítio nunca pode reprovar a suíte. O custo segue declarado — o mapa passa a descrever mais do que existe até ser podado à mão. O que é novo é que uma ocorrência **ainda existente** precisa casar com a **sua própria** forma pinada, exigência mais forte que o agregado por arquivo que ela substitui.
