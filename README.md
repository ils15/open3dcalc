...
| z-index  | Token `--z-dropdown: 60` da escala única em `src/styles/tokens.css` — acima da bottom nav mobile, abaixo do Tooltip. Ordem da escala: `--z-app-chrome: 20` < `--z-viewer: 30` < `--z-passive: 40` < `--z-shell-chrome: 45` < `z-50` (Tailwind, **só** `fixed inset-0` com fundo escurecido) < `--z-tour: 55` / `--z-tour-card: 56` < `--z-dropdown: 60` < `--z-panel: 70` < `--z-tooltip: 80` |
