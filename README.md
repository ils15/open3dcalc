...
| z-index  | Token `--z-dropdown: 60` da escala única em `src/styles/tokens.css` — acima da bottom nav mobile, abaixo do Tooltip. Ordem da escala: `--z-app-chrome: 20` < `--z-viewer: 30` < `--z-passive: 40` < `--z-shell-chrome: 45` < `z-50` (Tailwind, tier de modais **com scrim**) < `--z-dropdown: 60` < `--z-panel: 70` < `--z-tooltip: 80` |
...
> **Nota (escala de camadas):** a escala está documentada em `src/styles/tokens.css` e se consome como `style={{ zIndex: "var(--z-…)" }}` — o mesmo mecanismo do `Select`, que mantém o valor resolvível (e portanto testável) em vez de inferível de um literal. Ela é ordenada pelo que a superfície **possui**, não pelo tamanho, porque isso é que decide se uma superfície pode cobrir outra. **Nada passivo pode ocultar a saída**: uma superfície passiva não tem scrim, não prende foco, não é dona de tecla nenhuma e se descarta sozinha — é informação, não interrupção, então não tem por que esconder um controle, muito menos o único controle que encerra o Focus Mode. **Um modal com scrim pode ficar acima do chrome**, porque ele realmente é dono da tela até ser respondido (e é dono do Escape enquanto está aberto). Daí a divisão: `--z-passive` (toasts, popovers sem scrim, listboxes limitados) fica **abaixo** de `--z-shell-chrome`, a saída do Focus Mode; o tier `z-50` passa a significar apenas modal com scrim — antes ele também admitia os passivos, e era essa sobreposição que enterrava a saída (no `sm+` o toast e a saída compartilham a mesma âncora `sm:top-4 sm:right-4`). `--z-viewer` (overlay fullscreen e viewer de toolpath do `StlPreview`, ambos antes em `z-[100]`) fica abaixo do tier passivo para que um aviso nunca se perca atrás de um overlay de tela cheia, e `--z-app-chrome` é a bottom nav das duas plataformas — que tinham drifado para `z-50` e `z-40` do mesmo componente. `--z-panel` documenta o tier de painéis (`GuideDrawer`, `ManageVisibilityButton` — ainda em `z-[70]`/`z-[71]`). Única exceção declarada: `--z-tooltip` fica **acima** do chrome, porque a bolha é `pointer-events-none` e `visibility: hidden` quando fechada — pode sobrepor a saída visualmente com um trigger em hover, mas jamais interceptar um clique nela (e não pode ficar mais abaixo, já que o `FilamentInventory` renderiza um tooltip via `InputGroup` dentro do seu modal `z-50`).
