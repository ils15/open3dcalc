...
const SW_FIXTURE = [
  { full_name: "Bambu Lab P1S", manufacturer: "Bambu Lab", model: "P1S", technology: "FDM",
    specs: { build_volume_cm3: 16777, power_w: 1000, heater_power_w: 350 } },
  { full_name: "Anycubic PHOTON M3", manufacturer: "Anycubic", model: "PHOTON M3", technology: "SLA",
    specs: { build_volume_mm: { x: 163.8, y: 102.4, z: 180 }, power_w: 120 } },
  { full_name: "Elegoo SATURN3", manufacturer: "Elegoo", model: "SATURN3", technology: "SLA",
    specs: { build_volume_mm: { x: 218.9, y: 122.9, z: 250 }, nozzle_diameter_mm: 0.4 } },
  { full_name: "Creality Ender 3 V3 SE", manufacturer: "Creality", model: "Ender 3 V3 SE", technology: "FDM",
    specs: { build_volume_cm3: 12100, nozzle_diameter_mm: 0.4, max_speed_mm_s: 600, power_w: 270 } },
  { full_name: "Qidi X-Plus 3", manufacturer: "QIDI", model: "X-Plus 3", technology: "FDM",
    specs: { build_volume_mm: { x: 245, y: 200, z: 200 } } },
  // distractor — same brand, different model, must not match
  { full_name: "Creality K2", manufacturer: "Creality", model: "K2", technology: "FDM",
    specs: { build_volume_cm3: 24000 } },
  // SLS must map to resin
  { full_name: "Sinterit LISA", manufacturer: "Sinterit", model: "LISA", technology: "SLS",
...
describe("extractTechnical", () => {
  it("maps FDM and SLA/SLS technology", () => {
    expect(extractTechnical({ technology: "FDM", specs: {} }).technology).toBe("fdm");
    expect(extractTechnical({ technology: "SLA", specs: {} }).technology).toBe("resin");
    expect(extractTechnical({ technology: "SLS", specs: {} }).technology).toBe("resin");
    expect(extractTechnical({ technology: "UNKNOWN", specs: {} }).technology).toBeUndefined();
  });
  ...
  it("derives an approximate cube from cm³ when dimensions are absent", () => {
    const t = extractTechnical({ technology: "FDM", specs: { build_volume_cm3: 8000 } });
...
describe("matchPrinter", () => {
  ...
  it("matches despite casing and spacing variants", () => {
    const hit = matchPrinter({ brand: "Anycubic", name: "Photon M3" }, SW_FIXTURE);
  ...
  it("matches across brand case/spelling variants (QIDI vs Qidi)", () => {
    const hit = matchPrinter({ brand: "Qidi Tech", name: "X-Plus 3" }, SW_FIXTURE);
  ...
  it("picks the shortest prefix match to avoid over-matching a variant", () => {
    ...
    const hit = matchPrinter({ brand: "Creality", name: "Ender 3 V3 SE" }, SW_FIXTURE);
    expect(hit?.model).toBe("Ender 3 V3 SE");
  });

  it("returns null when only the brand matches", () => {
    const hit = matchPrinter({ brand: "Creality", name: "Ender 5 Neo" }, SW_FIXTURE);
    expect(hit).toBeNull();
  });
...
describe("enrich", () => {
  const appPrinter = {
    id: "bambu_p1s", brand: "Bambu Lab", name: "P1S",
    power: 350, value: 5500, usefulLife: 4000, maintenancePerHour: 0.4,
  };
  ...
  it("keeps an existing image untouched", () => {
    const out = enrich(
      { ...appPrinter, image: "/images/printers/brands/bambu-lab/bambu-lab-p1s-card-300.png" },
      matchPrinter(appPrinter, SW_FIXTURE),
    );
    expect(out.image).toBe("/images/printers/brands/bambu-lab/bambu-lab-p1s-card-300.png");
  });

  it("leaves technical fields absent when there is no match", () => {
    const p = { id: "m5", brand: "AnkerMake", name: "M5", value: 3500, power: 300 };
    const out = enrich(p, matchPrinter(p, SW_FIXTURE));
...
describe("fallbackImage", () => {
  it("honors explicit technology", () => {
    expect(fallbackImage({ technology: "resin" })).toBe("/images/printers/fallback-resin.svg");
    expect(fallbackImage({ technology: "fdm" })).toBe("/images/printers/fallback-fdm.svg");
  });

  it("infers resin from known model families", () => {
    expect(fallbackImage({ brand: "Elegoo", name: "Saturn 3" })).toBe("/images/printers/fallback-resin.svg");
    expect(fallbackImage({ brand: "Anycubic", name: "Photon M3" })).toBe("/images/printers/fallback-resin.svg");
    expect(fallbackImage({ brand: "Creality", name: "Halot Sky" })).toBe("/images/printers/fallback-resin.svg");
  });

  it("defaults to FDM", () => {
    expect(fallbackImage({ brand: "Creality", name: "Ender 3 V3" })).toBe("/images/printers/fallback-fdm.svg");
  });
});
...
describe("enrich — websiteUrl", () => {
  ...
  it("never overwrites an existing websiteUrl", () => {
    const out = enrich({ brand: "Prusa", name: "MK4", websiteUrl: "https://example.com" }, null);
