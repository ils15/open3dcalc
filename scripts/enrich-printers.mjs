...
const CATALOG_URL = 'https://raw.githubusercontent.com/swordlab/open-3d-printer-database/main/catalog.json'
...
const RESIN_PATTERNS = [
  'photon', 'halot', 'saturn', 'mars', 'sl1', 'sl2', 'sonic',
  'moai', 'phenom', 'forge', 'lcd', 'sla', 'msla', 'dlp',
  'forma', 'shining', 'shadow',
]
...
export function fallbackImage(printer) {
  if (printer.technology === 'resin') return `${FALLBACK_DIR}/fallback-resin.svg`
...
export function extractTechnical(source) {
  ...
  const out = {
    technology: source.technology === 'SLA' || source.technology === 'SLS'
      ? 'resin'
      : source.technology === 'FDM' ? 'fdm' : undefined,
    buildVolumeMm: specs.build_volume_mm
      ? { x: specs.build_volume_mm.x, y: specs.build_volume_mm.y, z: specs.build_volume_mm.z }
      : specs.build_volume_cm3 ? cubeFromCm3(specs.build_volume_cm3) : undefined,
...
export function matchPrinter(printer, sw) {
  ...
  for (const c of cands) {
    ...
    if (cm.startsWith(am) || am.startsWith(cm)) {
      if (!best || normalize(c.model).length < normalize(best.model).length) best = c
    }
  }
  return best
}
...
export function enrich(printer, source) {
  ...
  out.image = printer.image || fallbackImage({ ...printer, technology: out.technology })
  if (!out.websiteUrl && BRAND_URLS[printer.brand]) out.websiteUrl = BRAND_URLS[printer.brand]
  return out
}

async function fetchCatalog() {
  const res = await fetch(CATALOG_URL, { redirect: 'follow' })
  if (!res.ok) throw new Error(`Failed to fetch swordlab catalog: HTTP ${res.status}`)
...
function parsePrintersTs(source) {
  const re = /\{\s*id:\s*'([^']+)',\s*name:\s*'([^']+)',\s*brand:\s*'([^']+)'([^}]*)\}/g
  const rows = []
  let m
  while ((m = re.exec(source)) !== null) {
    ...
    const arr = (key) => {
      ...
      return hit[1].split(',').map(s => s.trim().replace(/^['"]|['"]$/g, '')).filter(Boolean)
    }
    rows.push({
      id: m[1], name: m[2], brand: m[3],
      power: num('power'), value: num('value'),
      usefulLife: num('usefulLife'),
      maintenancePerHour: num('maintenancePerHour'),
      image: str('image'), maxFilaments: num('maxFilaments'), tags: arr('tags'),
    })
  }
  return rows
}
...
function formatPrinter(p) {
  ...
  if (p.technology) parts.push(`    technology: ${p.technology ? JSON.stringify(p.technology) : undefined},`)
  if (p.buildVolumeMm) {
    const v = p.buildVolumeMm
    const approx = Math.abs(v.x - v.y) < 0.01 && Math.abs(v.y - v.z) < 0.01
    parts.push(`    // ${approx ? 'volume aproximado (∛cm³) — dimensões reais indisponíveis' : 'dimensões reais (mm)'}`)
    parts.push(`    buildVolumeMm: { x: ${round(v.x)}, y: ${round(v.y)}, z: ${round(v.z)} },`)
  }
  if (p.nozzleDiameterMm) parts.push(`    nozzleDiameterMm: ${p.nozzleDiameterMm},`)
  if (p.maxSpeedMmS) parts.push(`    maxSpeedMmS: ${p.maxSpeedMmS},`)
  if (p.websiteUrl) parts.push(`    websiteUrl: ${JSON.stringify(p.websiteUrl)},`)
  if (p.tags?.length) parts.push(`    tags: [${p.tags.map(t => JSON.stringify(t)).join(', ')}],`)
  return `  {\n${parts.join('\n')}\n  },`
}
...
export async function run({ write = false } = {}) {
  ...
  if (!rows.length) throw new Error('Could not parse printers.ts — bailing to avoid data loss.')
  if (rows.length !== 103) throw new Error(`Expected 103 printers, parsed ${rows.length}.`)
  ...
  console.log(`matched ${report.matched}/${rows.length} (unmatched: ${report.unmatched.length})`)
  ...
  if (missingImage.length) throw new Error('Every printer must have a thumbnail.')

  // Diff report: prove economic fields are untouched.
  const changed = enriched.map((p, i) => ({ id: p.id, tech: p.technology, build: p.buildVolumeMm }))
  console.log('\nSample (technical only):')
  for (const id of ['bambu_p1s', 'creality_ender_3_v3_se', 'anycubic_photon_m3', 'prusa_mk4', 'elegoo_saturn_3']) {
    const p = enriched.find(x => x.id === id)
    if (p) console.log(`  ${id}: tech=${p.technology} build=${JSON.stringify(p.buildVolumeMm)} nozzle=${p.nozzleDiameterMm} speed=${p.maxSpeedMmS}`)
  }
