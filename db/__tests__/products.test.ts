...
import { migrateDown, backupDatabase } from "../migrate.ts";
...
describe("migration 0002_products (up)", () => {
  ...
  it("creates helper indexes on name and sold", () => {
    const idx = sqlite.prepare(`PRAGMA index_list('products')`).all() as Array<{
      name: string;
    }>;
...
describe("products table (drizzle CRUD)", () => {
  ...
  it("marks a product as sold via placeholder-bound update (SQL-safe)", () => {
    const id = generateId();
    db.insert(schema.products)
      .values({ id, name: "Vaso Espiral", createdAt: NOW, updatedAt: NOW })
      .run();
    ...
    const count = (
      sqlite.prepare(`SELECT COUNT(*) AS n FROM products`).get() as {
        n: number;
      }
    ).n;
  ...
  it("toggles sold back to available (0)", () => {
    const id = generateId();
    db.insert(schema.products)
      .values({ id, name: "Toggle", sold: 1, createdAt: NOW, updatedAt: NOW })
      .run();
    db.update(schema.products)
      .set({ sold: 0 })
      .where(eq(schema.products.id, id))
...
describe("migration down + backup", () => {
  ...
  it("migrateDown() drops the products table (rollback)", () => {
    ...
    sqlite.exec(
      fs.readFileSync(
        path.join(__dirname, "..", "migrations", "0002_products.sql"),
        "utf-8",
      ),
    );
    void sql`SELECT 1`;
