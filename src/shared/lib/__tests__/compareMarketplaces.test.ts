...
function fdmFixture(
  ...
  return {
    ...
    ops: {
      ...
      ppeCostPerPrint: 0,
      carbonIntensity: 100,
    } satisfies OperationalCosts,
    soft: {
      enabled: false,
      slicerMonthlyCost: 0,
      modelFileCost: 0,
    } satisfies SoftwareCosts,
    hw: {
      enabled: false,
      nozzleEnabled: true,
      nozzleCost: 25,
...
describe("compareMarketplaceProfits", () => {
  ...
  it("runs over a custom catalog slice", () => {
    const slice = marketplaces.filter(
      (m) => m.id === "direct" || m.id === "etsy",
    );
    const rows = compareMarketplaceProfits({
      totalCost: 100,
      marginPercent: 30,
  ...
  it("scales with the base cost (risk/shipping are already inside totalCost)", () => {
    const base = compareMarketplaceProfits({
      totalCost: 100,
      marginPercent: 30,
      taxPercent: 0,
    });
    const doubled = compareMarketplaceProfits({
      totalCost: 200,
      marginPercent: 30,
      taxPercent: 0,
    });
