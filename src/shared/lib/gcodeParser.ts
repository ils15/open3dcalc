...
export function parseGcode(
  ...
  // The six extent headers are mapped ONCE in the pass below instead of being
  // looked up with `lines.find` at every `;MINX:` hit (each find rescanned the
  // whole file). MINX is last-wins — the old code re-evaluated the extents on
  // every hit — and the others are first-wins, matching find(). The extents
  // are computed after the loop so a header can sit anywhere in the file.
  let sawMinX = false;
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith(";MINX:")) {
      sawMinX = true;
      minX = parseFiniteFloat(trimmed.split(":")[1]);
    } else if (trimmed.startsWith(";MAXX:")) {
      if (maxX === undefined) maxX = parseFiniteFloat(trimmed.split(":")[1]);
    } else if (trimmed.startsWith(";MINY:")) {
      if (minY === undefined) minY = parseFiniteFloat(trimmed.split(":")[1]);
    } else if (trimmed.startsWith(";MAXY:")) {
      if (maxY === undefined) maxY = parseFiniteFloat(trimmed.split(":")[1]);
    } else if (trimmed.startsWith(";MINZ:")) {
      if (minZ === undefined) minZ = parseFiniteFloat(trimmed.split(":")[1]);
    } else if (trimmed.startsWith(";MAXZ:")) {
      if (maxZ === undefined) maxZ = parseFiniteFloat(trimmed.split(":")[1]);
    }
  ...
  if (sawMinX) {
    if (minX !== undefined && maxX !== undefined) metaSize.x = maxX - minX;
