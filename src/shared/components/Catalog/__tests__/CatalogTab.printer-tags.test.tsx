...
import { useCatalogStore, type CatalogPrinter } from '@/shared/stores/catalogStore'
...
const printer = (id: string, name: string, tags: string[] = [], custom = true): CatalogPrinter => ({
  id,
  name,
  brand: 'DIY',
  power: 120,
...
const seed = (printers: CatalogPrinter[]) => {
  localStorage.setItem(STORAGE_KEY, JSON.stringify({ printers, materials: [], marketplaces: [] }))
  useCatalogStore.setState({ printers, materials: [], marketplaces: [], selectedPrinterTag: null })
}
...
describe('CatalogTab — Printer tag organization', () => {
  ...
  it('renders one chip per unique tag', () => {
    seed([printer('p1', 'Alpha', ['resin', 'fast']), printer('p2', 'Beta', ['resin'])])
    render(<CatalogTab />)
    ...
    expect(screen.getByRole('button', { name: 'catalog.allTags' })).toBeDefined()
  })
  ...
  it('filters printers when a tag chip is clicked', () => {
    ...
    expect(screen.getByRole('button', { name: 'fdm' })).toHaveAttribute('aria-pressed', 'false')
  })
  ...
  it('clears the filter via the "All" chip', () => {
    ...
    expect(screen.getByText('Alpha')).toBeDefined()
    expect(screen.getByText('Beta')).toBeDefined()
    expect(screen.getByRole('button', { name: 'catalog.allTags' })).toHaveAttribute('aria-pressed', 'true')
  })
  ...
  it('only renders the tag editor for custom printers', () => {
    seed([printer('p1', 'Alpha', ['resin']), printer('p2', 'Beta', ['resin'], false)])
    render(<CatalogTab />)
  ...
  it('adds a normalized tag via Enter', () => {
    ...
    expect(useCatalogStore.getState().printers.find(p => p.id === 'p1')?.tags).toEqual(['voron 2'])
  ...
  it('keeps the Add button disabled until input is non-empty', () => {
    ...
    const button = screen.getByRole('button', { name: 'catalog.addTag' }) as HTMLButtonElement
