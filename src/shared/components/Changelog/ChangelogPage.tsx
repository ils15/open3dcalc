...
export function ChangelogPage() {
  ...
  const changelogData: VersionEntry[] = (
    Array.isArray(raw) ? (raw as VersionEntry[]) : []
  )
    .slice()
    ...
    <div className="space-y-5">
      <div className="surface rounded-xl p-5 text-center sm:p-6">
        ...
        <p className="text-xs text-[var(--color-text-secondary)]">
          {t("changelog.summary", {
            versions: t("changelog.versionsCount", {
      ...
      <div className="relative space-y-4">
        {changelogData.map((entry, index) => {
          ...
          return (
            <div
              key={entry.version}
              className="surface min-w-0 overflow-hidden rounded-xl transition-all"
            >
              <button
              ...
              >
                <div
                  data-testid={`version-heading-${entry.version}`}
                  ...
                  {formattedDate && (
                    <>
                      ...
                      <time
                        ...
                        dateTime={entry.date}
                        aria-label={t("changelog.releaseDate", {
                          date: formattedDate,
                        })}
                    ...
                    <div key={section.title} className="min-w-0">
                      ...
                      <ul className="space-y-1.5">
                        {section.items.map((item, itemIndex) => (
                          <li
                            key={itemIndex}
                            className="flex min-w-0 items-start gap-2 break-words pl-1 text-xs text-[var(--color-text-secondary)]"
                          >
                            <span
                              className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--color-accent)]/50"
                              aria-hidden="true"
                            />
