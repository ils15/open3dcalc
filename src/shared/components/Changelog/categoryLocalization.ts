...
const RAW_CATEGORY_ALIASES: Record<string, ChangelogCategoryKey> = {
  features: "features",
  feature: "features",
...
const CATEGORY_ALIASES = Object.fromEntries(
  Object.entries(RAW_CATEGORY_ALIASES).map(([alias, key]) => [
    normalizeCategoryTitle(alias),
    key,
  ]),
) as Record<string, ChangelogCategoryKey>;

export function normalizeCategoryKey(
  title: string,
): ChangelogCategoryKey | undefined {
...
export function getLocalizedCategoryTitle(
  ...
  return translatedTitle && translatedTitle !== translationKey
    ? translatedTitle
    : title;
}
