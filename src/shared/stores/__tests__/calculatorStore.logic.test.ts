  ...
  describe("AMS logic", () => {
    ...
    it("AMS enabledSlots counts only enabled slots", () => {
      ...
      const slots = store.fdmAmsSlots.map((s, i) => ({
        ...s,
        enabled: i < 2,
        color: ["#ff0000", "#00ff00", "#0000ff"][i] || s.color,
      }));
      slots.forEach((slot, i) => store.setFdmAmsSlot(i, slot));
      const enabledSlots = useCalculatorStore
        .getState()
        .fdmAmsSlots.filter((sl) => sl.enabled);
      expect(enabledSlots).toHaveLength(2);
    });

    it("Wave B: fills fdmMachine costs from the printer catalog on selection", () => {
      const store = useCalculatorStore.getState();
      // bambu_a1: value 3000, usefulLife 3000, maintenancePerHour 0.30
      store.setSelectedPrinter({
        id: "bambu_a1",
        name: "A1",
        brand: "Bambu Lab",
      ...
      const m = useCalculatorStore.getState().fdmMachine;
      const hpm = m.hoursPerMonth;
      expect(m.machineCost).toBe(3000);
      expect(m.depreciationMonths).toBe(Math.round(3000 / hpm));
      expect(m.maintenanceEnabled).toBe(true);
      // R$/h -> R$/mes: 0.30 * 200 = 60
      expect(m.maintenanceCost).toBe(Math.round(0.3 * hpm));
    });

    it("Wave B: fills the RESIN machine when the resin tab is active", () => {
      const store = useCalculatorStore.getState();
      store.setActiveTab("resin");
      // anycubic_photon_m3: value 2000, usefulLife 2000, maintenancePerHour 0.15
      store.setSelectedPrinter({
      ...
      const m = useCalculatorStore.getState().resinMachine;
      const hpm = m.hoursPerMonth;
      expect(m.machineCost).toBe(2000);
      expect(m.depreciationMonths).toBe(Math.round(2000 / hpm));
      expect(m.maintenanceEnabled).toBe(true);
      expect(m.maintenanceCost).toBe(Math.round(0.15 * hpm));
      // FDM machine untouched on the resin tab
      expect(useCalculatorStore.getState().fdmMachine.machineCost).not.toBe(
        2000,
      );
    });

    it("Wave B: also updates the active tab print power", () => {
      const store = useCalculatorStore.getState();
      store.setSelectedPrinter({
        id: "bambu_a1",
        name: "A1",
      ...
      expect(
        useCalculatorStore.getState().fdmPrintParams.printerPowerWatts,
      ).toBe(220);
      ...
      expect(
        useCalculatorStore.getState().resinPrintParams.printerPowerWatts,
      ).toBe(120);
    });

    it("Wave B: clamps depreciationMonths to a minimum of 1", () => {
      const store = useCalculatorStore.getState();
      // usefulLife 10 / hoursPerMonth 200 -> 0.05 -> round 0 -> clamp 1
      store.setSelectedPrinter({
        id: "tiny",
        name: "Tiny",
        brand: "X",
    ...
    it("Wave B (derive-once): later hoursPerMonth edits do NOT re-derive machine costs", () => {
      const store = useCalculatorStore.getState();
      store.setSelectedPrinter({
        id: "bambu_a1",
        name: "A1",
      ...
      const afterSelect = useCalculatorStore.getState().fdmMachine;
      const derivedMonths = afterSelect.depreciationMonths;
      const derivedMaintenance = afterSelect.maintenanceCost;

      // Usuario muda horas/mes DEPOIS: a derivada da selecao e preservada
      store.setFdmMachine({ ...afterSelect, hoursPerMonth: 500 });

      const m = useCalculatorStore.getState().fdmMachine;
      expect(m.hoursPerMonth).toBe(500);
      expect(m.machineCost).toBe(3000);
      expect(m.depreciationMonths).toBe(derivedMonths);
      expect(m.maintenanceCost).toBe(derivedMaintenance);
    });
  });
});
