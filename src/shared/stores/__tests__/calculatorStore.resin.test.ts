...
describe("CalculatorStore Resin", () => {
  ...
  describe("computeStoreResults path", () => {
    ...
    it("resin post-processing washing disabled → no alcohol cost", () => {
      ...
      store.setActiveTab("resin");
      const ppBefore =
        useCalculatorStore.getState().results!.postProcessingCost;
  ...
  describe("sell price edge cases", () => {
    it("resin feePercent >= 1 → sellPrice uses priceBeforeFees * 2", () => {
      ...
      const profitAmountRaw = totalBaseCost * 0.1;
  ...
  // ══════════════════════════════════════════════════════════════
  //  Wave B — water-washable auto-switch on material selection
  // ══════════════════════════════════════════════════════════════
  ...
  describe("setResinMaterial washType auto-switch", () => {
    it('water-washable resin (UI display name) → washType "water" and zero IPA cost', () => {
      ...
      const after = useCalculatorStore.getState();
      expect(after.resinPostProcess.washType).toBe("water");

      // 0.1 L de IPA a 25/L jamais entra: lavagem com água custo 0.
      expect(after.results!.postProcessingCost).toBe(0);
    });
    ...
    it('regular resin (UI display name) → washType "alcohol"', () => {
      ...
      store.setResinPostProcess({
        ...
        alcoholVolumeLiters: 0.1,
        alcoholCostPerLiter: 25,
        curingEnabled: false, // isola o custo de lavagem
      });
      ...
      // álcool volta a entrar: 0.1 L * 25/L = 2.5
      expect(after.results!.postProcessingCost).toBeCloseTo(2.5, 5);
    });

    it("absent washType in a legacy payload ≡ alcohol (byte-identical)", () => {
      localStorage.clear();
      const legacy = {
        ...initialState,
      ...
      expect(
        useCalculatorStore.getState().resinPostProcess.washType,
      ).toBeUndefined();

      // setState raw não recomputa resultados; força o path setWithCompute
      useCalculatorStore.getState().setActiveTab("resin");

      // Sem washType ⇒ calculateResin trata como álcool (IPA presente: 2.5)
      expect(
        useCalculatorStore.getState().results!.postProcessingCost,
      ).toBeCloseTo(2.5, 5);
    });
